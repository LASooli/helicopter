...
iy = 5313                       # kg m^2) Helicopter moment of inertia in the pitch axis 
mast = 1.24                     # %(m) Vertical distance between rotor CG and rotor hub
omega = vtip / (diam / 2)       # Rotor tip speed
area = np.pi / 4 * diam ** 2    # Rotor area
tau = .1                        # (sec) time constant in dynamics inflow!!!
collect = [5.54*np.pi/180]      # (rad)  Collective pitch angle
longit = [2.49*np.pi/180]        # (rad) Longitudinal cyclic pitch angle
...
pitch0 = -5 * np.pi / 180                    #(rad) Setting initial helicopter pitch angle
u0 = 90 * 0.514444 #* np.cos(pitch0)                     #(m/sec) Setting initial helicopter airspeed component along body x-axis 
...
for i in range(aantal):
    ...
    if 130 > ti >= 60:
        vh_des = V_knot1 * 0.514444
        #if vh_des > V_knot1 * 0.514444:
         #   vh_des -= 1* 0.514444              # Created this to make steady changes to velocity to make smoother
    elif 180 > ti >= 130:
        #if vh_des < V_knot2 * 0.514444:
        #    vh_des += 1*V_knot2* 0.514444      #Replaced this with an integral controller
        vh_des = V_knot2 * 0.514444
    elif ti >= 180:
        vh_des = V_knot3 * 0.514444
        
        
        # -- PID - Controller:
    u_des = vh_des * np.sin(pitch[-1])
    
    vh_err = vh_des - vh[-1]
    vh_err_accum += vh_err                             # Integral controller 
    pitch_des = -0.03*(vh_err) - 0.005*w[-1] - 0.001 * vh_err_accum
                 #P-control       #D-control      #I-control

    longitgrd = 0.5 * (pitch[-1]-pitch_des) * 180 / np.pi #+ 0.2 * (q[-1] * 180 / np.pi)
                        # P-control                             #D-control
    ...
    c_err  = c_des - c
    c_err_accum += c_err
    collectgrd = C1 * (c_err) + C2 * c_err_accum
                    #P - control     #I-control
    ...
    """Equations of motion"""
    udot = -g * np.sin(pitch[-1]) - cds / mass * .5 * rho * u[-1] * vv * S + thrust / mass * np.sin(helling) - q[-1] * w[-1]
    wdot = g * np.cos(pitch[-1]) - cds / mass * .5 * rho * w[-1] * vv * S - thrust / mass * np.cos(helling) + q[-1] * u[-1]
    ...
    vhdot = udot * np.cos(pitch[-1]) + wdot * np.sin(pitch[-1])
...
plt.figure()
plt.title('w and Pitch over Time')
plt.xlabel('t (s)')
plt.plot(t, w, label="w (m/s)", color="b") 
plt.plot(t, np.array(pitch) * 180 / np.pi, label="pitch (deg)", color="c") 
plt.legend()
plt.grid()
plt.show()
...
plt.figure()
plt.title('Velocity Horizontal over Time')
plt.ylabel(' vh (m/s)')
plt.xlabel('t (s)')
plt.plot(t, np.array(vh)/0.514444, label="vh (knot)", color="b") 
plt.legend()
plt.grid()
plt.show()
...
plt.plot(t, np.array(pitch) * 180 / np.pi, label="pitch (deg)", color="c") 
plt.legend()
#lt.xlim(xmin=20, xmax=120)  # Adjust the limits for the x axis
plt.ylim(ymin=-20, ymax=120)  # Adjust the limits for the y axis
plt.grid()
plt.show()
