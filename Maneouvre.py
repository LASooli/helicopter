...
iy = 5313                       # kg m^2) Helicopter moment of inertia in the pitch axis \\\\\Assumed value
...
for i in range(aantal):
    ...
    if 130 > ti >= 60:
        ...
        #    vh_des += 1*V_knot2* 0.514444
   # elif 140 > ti >= 110:
        vh_des = V_knot2 * 0.514444
    elif ti >= 180:
        vh_des = V_knot3 * 0.514444
    #if not u[-1] == 0:
        #vh_ = u[-1] * np.cos(pitch[-1]) + w[-1] * np.sin(pitch[-1])
    #print(vh_-u[-1])
    #vh.append(vh_/0.514444)
    ...
    pitch_des = -0.02*(vh_des-vh[-1]) - 0.01*w[-1]
    #pitch_des = -0.08#-0.005 * (u_des-u[-1]) #- 0.01*w[-1]
    #pitch_des = -0.02*(u_des-u[-1])-0.001*du[-1] - 0.01*w[-1] #- 0.1*du[-1] #+ 0.02 * u[-1]
    #pitch_des = -5*np.pi/180
    #if ti >= 15:
    longitgrd = 1 * (pitch[-1]-pitch_des) * 180 / np.pi + 1 * (q[-1] * 180 / np.pi)  # PD in deg
